...
public class GLRendererV9 implements GLSurfaceView.Renderer {
    ...
    private Bitmap grayscaleHeightMap;

    public GLRendererV9(Context context) {
...
//        fpvPosition[1] = getHeightFromBitmap(0, 0) + 2.0f;
        ...
        // 转换为灰度高度图
        if (tiffBitmap != null) {
            grayscaleHeightMap = TextureUtils.convertToGrayscaleManual(tiffBitmap);
            Log.i(TAG, "Grayscale height map created");
        }
    }
...
//        Log.i(TAG, "Height map texture loaded: " + heightMapTextureId);
        ...
        // 创建高度图纹理
        if (grayscaleHeightMap != null) {
            heightMapTextureId = TextureUtils.createHeightMapTexture(grayscaleHeightMap);
        }
